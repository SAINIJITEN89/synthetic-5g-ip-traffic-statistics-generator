...
# Using enhanced configuration with all features
python trafficgen.py --config enhanced_config.yaml

# Custom output format and location
python trafficgen.py --duration 30 --users 500 --format json --output results.json
```
...
- **CSV/JSON**: Detailed per-interval statistics with all metrics
- **Summary**: Aggregate statistics, traffic breakdown, and performance metrics
- **Report**: Human-readable summary with key insights
- **Per-Profile Data**: Individual statistics for each traffic class
- **Impairment Metrics**: Loss rates, latency, jitter per interval and profile
- **Time-of-Day Analysis**: Traffic multipliers and pattern analysis
...
```
--config CONFIG     Path to YAML/JSON configuration file
...
```bash  
# Special event simulation with enhanced config
python trafficgen.py --config enhanced_config.yaml \
    --output special_event_analysis.csv
```

### Multi-Profile Performance Testing
```bash
# Test different traffic mixes for capacity planning
python trafficgen.py --config multi_profile_test.yaml \
    --format json --output capacity_test.json
```
